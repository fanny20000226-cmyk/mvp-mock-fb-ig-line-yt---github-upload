(function () {
  var MENU = [
    ...
    { label: "預約評估", children: [
      ...
      { label: "後台評估", action: "data-go=\"eval\"", views: ["eval"] }
    ] },
    { label: "製作報價單", children: [
      { label: "報價介面", action: "data-make-quote-front", quote: true },
      { label: "報價單列表", action: "data-quote-page", quote: true }
    ] },
    { label: "施工開單", children: [
      { label: "施工訂單", action: "data-go=\"orders\"", views: ["orders"] },
      { label: "今日行事曆", action: "data-go=\"calendar\"", views: ["calendar"] },
      { label: "工單追溯", action: "data-quote-workorders", quote: true }
    ] },
    { label: "車輛相簿", action: "data-go=\"customers\"", views: ["customers"] },
    { label: "財務報表", children: [
      { label: "營收報表", action: "data-go=\"reports\"", views: ["reports"] },
    ...
    { label: "系統設定", children: [
      ...
      { label: "價格設定", action: "data-go=\"prices\"", views: ["prices"] },
      { label: "操作紀錄", action: "data-go=\"logs\"", views: ["logs"] }
    ] }
  ];
  ...
  function plan(order) {
    ...
    return plans.filter(function (item) { return item.id === order.plan; })[0] || { name: "內裝清潔", price: 0 };
  }
  ...
  function renderSidebar() {
    ...
    var sidebar = "<aside class=\"cc-sidebar\" data-carcare-shell>" +
      ...
      "</aside>";
    if (aside) aside.outerHTML = sidebar;
    else {
  ...
  function renderContentBar() {
    ...
    var html = "<section class=\"cc-contentbar\"><div><h1>" + esc(title) + "</h1><small>CarCare System 門店管理後台</small></div><div class=\"cc-content-actions\"><strong>今日預約 " + count + "</strong><button aria-label=\"通知\">通知</button><span class=\"cc-avatar\">店長</span><button aria-label=\"縮小\">−</button></div></section>";
  ...
  function lineRows(orders) {
    var fallback = [
      { id: "demo-1", name: "王生", store: "三重門市", plate: "ABC-1234", car: "Altis", time: "09:30", status: "待確認", planName: "內裝清潔" },
      { id: "demo-2", name: "DEF-5678", store: "桃園門市", plate: "DEF-5678", car: "Altis", time: "11:00", status: "待施工", planName: "打蠟保養" },
      { id: "demo-3", name: "李小姐", store: "新竹門市", plate: "GHI-9012", car: "BMW", time: "14:30", status: "待提醒", planName: "皮革護理" }
    ];
    ...
    return list.map(function (order) {
      ...
      return "<article class=\"cc-appointment-row\"><div><b>" + esc(order.name || order.plate || "車主") + "</b><small>" + esc(order.plate || order.car || "") + "</small></div><span>" + esc(order.store || "三重門市") + "</span><span>" + esc(order.time || "09:30") + "</span><span>" + esc(service) + "</span><button" + action + ">操作</button></article>";
    }).join("");
  }

  function detailCard(title, order) {
    var selected = order || {};
    return "<section class=\"cc-panel\"><h2>" + esc(title) + "</h2><dl class=\"cc-detail-list\"><div><dt>車主</dt><dd>" + esc(selected.name || "王生") + "</dd></div><div><dt>車牌</dt><dd>" + esc(selected.plate || "DEF-5678") + "</dd></div><div><dt>服務項目</dt><dd>" + esc((selected.planName || (selected.plan ? plan(selected).name : "")) || "內裝清潔") + "</dd></div><div><dt>備註</dt><dd>" + esc(selected.note || "到店前 30 分鐘發送提醒") + "</dd></div></dl><button class=\"cc-primary-action\">發送LINE提醒</button></section>";
  }
  ...
  function renderLineDashboard() {
    ...
    var dashboard = "<section class=\"cc-line-dashboard\">" +
      ...
      "<section class=\"cc-panel cc-timeline\"><h2>今日排程時間軸</h2><div><p><i></i><b>09:30</b><span>王生 / 內裝清潔</span></p><p><i></i><b>11:00</b><span>DEF-5678 / 打蠟保養</span></p><p><i></i><b>14:30</b><span>李小姐 / 皮革護理</span></p></div></section>" +
      "</section>";
