...
export default function FieldModePage() {
  ...
  return <RequireAuth allow={["admin", "shop_manager", "vice_manager", "worker"]}><main className="field-mode space-y-4">
    ...
    <div className="grid gap-4 lg:grid-cols-2">{rows.map((row) => <article key={row.id} className="rounded-xl border border-neutral-300 bg-white p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-black text-neutral-500">{row.order_no}</p><h2 className="text-2xl">{row.cars?.plate_no || "未填車牌"}</h2><p className="text-base font-bold text-neutral-700">{row.cars?.customer_name || "未命名客戶"}</p></div><span className="status-chip status-info">● {row.status === "working" ? "施工中" : row.status === "finished" ? "已完工" : "待施工"}</span></div><div className="my-4 rounded-xl bg-neutral-100 p-4"><strong>工單備註</strong><p className="mt-1 whitespace-pre-wrap text-base text-neutral-700">{row.remark || "無備註"}</p></div><WorkOrderPhotoUploader orderId={row.id} compact /><div className="mt-3">{row.status === "working" ? <button type="button" className="primary-btn field-big-button w-full" onClick={() => requestFinish(row)}><CheckCircle2 />完成施工</button> : row.status === "finished" ? <button type="button" className="secondary-btn field-big-button w-full" disabled>已完成</button> : <button type="button" className="primary-btn field-big-button w-full" disabled={Boolean(running)} onClick={() => setStatus(row, "working")}><Play />開始施工</button>}</div></article>)}</div>
