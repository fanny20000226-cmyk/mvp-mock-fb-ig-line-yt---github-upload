...
export default function ConstructionPage() {
  ...
  async function load() {
    const { data, error } = await listConstructionOrders();
  ...
  async function updateStatus(row: OrderRow, status: string) {
    ...
    if (status === "ready_pickup") await promptPickupNotice(row);
    await load();
  }
  ...
  async function promptPickupNotice(row: OrderRow) {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : "建立通知紀錄失敗。");
    }
  }
    ...
    <RequireAuth>
      <section className="card">
        <div className="mb-5">
          ...
          <p className="mt-1 text-sm text-neutral-500">管理待確認、施工中、已完工與取消訂單。</p>
        </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-4">
          ...
          <div className="rounded-2xl border border-neutral-200 bg-white p-4">
            ...
            <p className="mt-2 text-3xl font-black text-carcare-yellow">{rows.filter((row) => row.status === "working").length}</p>
          </div>
          <div className="rounded-2xl border border-neutral-200 bg-white p-4">
            <p className="text-sm text-neutral-500">已完工</p>
            <p className="mt-2 text-3xl font-black text-carcare-yellow">{rows.filter((row) => row.status === "finished").length}</p>
          </div>
          <div className="rounded-2xl border border-neutral-200 bg-white p-4">
            <p className="text-sm text-neutral-500">待收尾款</p>
            <p className="mt-2 text-3xl font-black text-carcare-yellow">{rows.filter((row) => balance(row) > 0).length}</p>
          </div>
        </div>
        ...
        <div className="table-wrap mt-5">
          <table className="data-table">
            ...
            <tbody>
                  ...
                  <Fragment key={row.id}>
                    <tr>
                      <td>
                        ...
                        <p className="text-xs text-neutral-500">來源報價：{row.quotations?.quote_no || "-"}</p>
                      </td>
                      <td>
                        <p className="font-black text-neutral-900">{row.cars?.customer_name || "-"}</p>
                        <p className="text-xs text-neutral-500">{row.cars?.plate_no || row.cars?.license_plate || "-"}</p>
                        <p className="text-xs text-neutral-500">{carName(row)}</p>
                      </td>
                      <td>
                        <select className="form-input min-w-32" value={row.status} onChange={(event) => updateStatus(row, event.target.value)}>
                          <option value="pending">待確認</option>
                          <option value="scheduled">已排程</option>
                          <option value="working">施工中</option>
                          <option value="finished">已完工</option>
                          <option value="ready_pickup">完工待牽車</option>
                      ...
                      <td>
                        <p>{money(Number(row.paid_amount || 0))}</p>
                        <p className="text-xs text-neutral-500">尾款 {money(Math.max(balance(row), 0))}</p>
                      </td>
                      <td>
                        <div className="flex flex-wrap gap-2">
                          <button className="secondary-btn" onClick={() => updateStatus(row, "working")} disabled={row.status === "working" || row.status === "finished"}>
                            開始
                          </button>
                          <button className="primary-btn" onClick={() => updateStatus(row, "ready_pickup")} disabled={row.status === "ready_pickup" || row.status === "picked_up"}>
                            完工待牽車
                          </button>
                          <button className="secondary-btn" onClick={() => updateStatus(row, "picked_up")} disabled={row.status === "picked_up"}>
                            已牽車
                          </button>
                          <button className="secondary-btn" onClick={() => toggleExpanded(row, isExpanded)}>
                            {isExpanded ? "收合" : "明細"}
                          </button>
                        </div>
                      </td>
                        ...
                        <td colSpan={8}>
                          <div className="rounded-2xl border border-neutral-200 bg-white p-5">
                            <div className="flex flex-wrap items-start justify-between gap-4 border-b border-neutral-200 pb-4">
                              <div>
                                ...
                                <p className="mt-1 text-sm text-neutral-500">來源報價：{row.quotations?.quote_no || "-"}</p>
                              </div>
                              <div className="text-right">
                                <p className="text-sm text-neutral-500">目前狀態</p>
                                <p className="text-xl font-black text-carcare-yellow">{statusText[row.status] || row.status}</p>
                              </div>
                            </div>

                            <div className="mt-5 grid gap-4 md:grid-cols-2">
                              <div className="rounded-2xl bg-neutral-50 p-4">
                                <h3 className="font-black">客戶車輛</h3>
                                <p className="mt-3">車主：{row.cars?.customer_name || "-"}</p>
                                <p>電話：{row.cars?.customer_phone || "-"}</p>
                                <p>車牌：{row.cars?.plate_no || row.cars?.license_plate || "-"}</p>
                                <p>車型：{carName(row)}</p>
                              </div>
                              <div className="rounded-2xl bg-neutral-50 p-4">
                                <h3 className="font-black">施工與金額</h3>
                                <p className="mt-3">開始：{row.start_at || "-"}</p>
                                <p>完工：{row.finish_at || "-"}</p>
                                <p>施工總額：{money(Number(row.total_amount || 0))}</p>
                                <p>已收金額：{money(Number(row.paid_amount || 0))}</p>
                                <p className="font-black text-carcare-yellow">剩餘尾款：{money(Math.max(balance(row), 0))}</p>
                              </div>
                            </div>
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            <PhotoZipButton
                              ...
                              filename={`PEIWAY_${row.cars?.plate_no || row.cars?.license_plate || row.order_no}_${String(row.finish_at || row.start_at || new Date().toISOString()).slice(0, 10)}`}
                            />
                            <ReceiptExportButton
                              source={{
                                quotationId: row.quotations?.id || null,
                                ...
                                plateNo: row.cars?.plate_no || row.cars?.license_plate || null,
                            ...
                            />
                            <button type="button" className="primary-btn" onClick={() => exportWorkOrderPdf(row)} disabled={exportingId === row.id}>
                              {exportingId === row.id ? "匯出中..." : "匯出施工工單 PDF"}
                            </button>
                          </div>
                        </td>
                      </tr>
              ...
              {!rows.length ? (
                <tr>
                  <td colSpan={8} className="text-center text-neutral-500">尚未建立施工訂單。</td>
                </tr>
              ) : null}
            </tbody>
          </table>
