...
export async function listConstructionOrders() {
  return supabase
    ...
    .select(
      "id, shop_id, order_no, status, start_at, finish_at, total_amount, paid_amount, remark, cars(customer_name, customer_phone, plate_no, brand, model, year), quotations(id, quote_no, final_amount, remark, status)"
    )
    .order("created_at", { ascending: false });
}
